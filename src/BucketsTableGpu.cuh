...
class BucketsTableGpu {
   ...
   public:
    ...
    struct __align__(alignof(TagType)) Bucket {
        ...
        __device__ bool containsWarp(TagType tag) const {
            ...
            return __ballot_sync(0xffffffff, found) != 0;
        }
   ...
   public:
    explicit BucketsTableGpu() {
        ...
        CUDA_CALL(cudaMalloc(&d_numOccupied, sizeof(size_t)));
    ...
    void clear() {
        ...
        CUDA_CALL(cudaMemset(d_numOccupied, 0, sizeof(size_t)));
        h_numOccupied = 0;
    }
    ...
    struct DeviceTableView {
        ...
        __device__ bool tryInsertAtomically(size_t bucketIdx, TagType tag) {
            for (int i = 0; i < bucketSize; ++i) {
                if (atomicCAS(&d_buckets[bucketIdx].tags[i], EMPTY, tag) ==
                    EMPTY) {
                    atomicAdd(
        ...
        __device__ bool insertWithEviction(TagType fp, size_t startBucket) {
            ...
            for (size_t evictions = 0; evictions < maxProbes; ++evictions) {
                if (tryInsertAtomically(currentBucket, currentFp)) {
                    return true;
                }
                ...
                TagType evictedFp = atomicExch(
                    &d_buckets[currentBucket].tags[evictSlot], currentFp
                );
        ...
        __device__ bool insert(const T& key) {
            ...
            if (tryInsertAtomically(h1, fp) || tryInsertAtomically(h2, fp)) {
                return true;
            }
    ...
    DeviceTableView get_device_view() {
        return DeviceTableView{d_buckets, d_rand_states, d_numOccupied};
    }
};
